...
public class RegionListGateway<T> implements Gateway<T> {
 
	private static int index = 1;
	private List<T> list = new ArrayList<T>();

	public void add(T entity) throws SQLException {
		((Region) entity).setID(index++);
		list.add(entity);
	}

	public void modify(T entity) throws SQLException {
		int id = ((Region) entity).getID();
		if (get(null, id) != null) {
			remove(get(null, id));
			add(entity);
		} else {
	...
	public Collection<T> getAllBy(Class className, int parentID) throws SQLException {
		...
		for (int i = 0; i < size(); i++) {
			if (((Region) list.get(i)).getParentID() == parentID) {
