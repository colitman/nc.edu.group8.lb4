...
public class UniversityBean implements EntityBean {
	...
	private static final String CREATE_QUERY = "insert into " + TABLE_NAME + " (PARENT_ID, ID, NAME, DEPTS_COUNT, WWW) values (?, ?, ?, ?, ?)";
	private static final String LOAD_QUERY = "select PARENT_ID, NAME, DEPTS_COUNT, WWW from " + TABLE_NAME + " where ID = ?";
	private static final String REMOVE_QUERY = "delete from " + TABLE_NAME + " where ID = ?";
	private static final String UPDATE_QUERY = "update " + TABLE_NAME + " set PARENT_ID = ?, NAME = ?, DEPTS_COUNT = ?, WWW = ? where ID = ?";
